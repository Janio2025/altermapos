...
class Financeiro extends MY_Controller
{
    ...
    public function lancamentos()
    {
        ...
        if (! empty($usuario_id)) {
            if (empty($where)) {
                $where = "(lancamentos.usuarios_id = '" . $usuario_id . "' OR vendas.usuarios_id = '" . $usuario_id . "')";
            } else {
                $where .= " AND (lancamentos.usuarios_id = '" . $usuario_id . "' OR vendas.usuarios_id = '" . $usuario_id . "')";
            }
        }
        ...
        // Adiciona filtro para considerar ou não lançamentos de OS
        if (!$considerar_os) {
            if (empty($where)) {
                $where = "os_id IS NULL";
            } else {
                $where .= " AND os_id IS NULL";
            }
        }

        $this->load->library('pagination');

        $this->data['configuration']['base_url'] = site_url("financeiro/lancamentos/?vencimento_de=$vencimento_de&vencimento_ate=$vencimento_ate&cliente=$cliente&tipo=$tipo&status=$status&periodo=$periodo&usuario=$usuario&usuario_id=$usuario_id&considerar_os=$considerar_os");
        $this->data['configuration']['total_rows'] = $this->financeiro_model->count('lancamentos', $where, 'usuarios, vendas');
        $this->data['configuration']['page_query_string'] = true;

        $this->pagination->initialize($this->data['configuration']);

        $this->data['results'] = $this->financeiro_model->get('lancamentos', '*', $where, $this->data['configuration']['per_page'], $this->input->get('per_page'), 'usuarios, vendas');
        $this->data['totals'] = $this->financeiro_model->getTotals($where, !empty($usuario_id) ? 'usuarios, vendas' : 'usuarios');
