<style>
  ...
  @media (max-width: 768px) {
    table#tabela-vendas thead {
      display: none;
    }

    table#tabela-vendas tbody tr {
      display: block;
      margin-bottom: 15px;
    ...
    table#tabela-vendas tbody tr td {
      display: flex;
    ...
    table#tabela-vendas tbody tr td::before {
    ...
    table#tabela-vendas tbody tr td:last-child {
      display: flex;
      ...
      <table id="tabela-vendas" class="table table-bordered">
        ...
        <tbody>
          ...
          foreach ($results as $r) {
            ...
            echo '<tr>';
            echo '<td data-label="Nº">' . $r->idVendas . '</td>';
            echo '<td data-label="Cliente"><a href="' . base_url() . 'index.php/clientes/visualizar/' . $r->idClientes . '">' . $r->nomeCliente . '</a></td>';
            echo '<td data-label="Vendedor">' . $r->nome . '</td>';
            echo '<td data-label="Data da Venda">' . $dataVenda . '</td>';
            echo '<td data-label="Venc. da Garantia"><span class="badge" style="background-color: ' . $corGarantia . '; border-color: ' . $corGarantia . '">' . $vencGarantia . '</span> </td>';

            if ($r->faturado == 1) {
              echo '<td data-label="Valor Total">R$ ' . number_format($r->valorTotal, 2, ',', '.') . '</td>';
              echo '<td data-label="Desconto">R$ ' . number_format($r->desconto, 2, ',', '.') . '</td>';
              echo '<td data-label="Valor com Desconto">R$ ' . number_format($r->valor_desconto, 2, ',', '.') . '</td>';
              echo '<td data-label="V. T. (Faturado)">R$ ' . number_format($r->valor_desconto, 2, ',', '.') . '</td>';
            } else {
              ...
              echo '<td data-label="Valor Total">R$ ' . number_format($valorProdutos, 2, ',', '.') . '</td>';
              echo '<td data-label="Desconto">R$ ' . number_format($desconto, 2, ',', '.') . '</td>';
              echo '<td data-label="Valor com Desconto">R$ ' . number_format($valorComDesconto, 2, ',', '.') . '</td>';
              echo '<td data-label="V. T. (Faturado)">R$ 0,00</td>';
            }

            echo '<td data-label="Status"><span class="badge" style="background-color: ' . $corStatus . '; border-color: ' . $corStatus . '">' . $r->status . '</span> </td>';
            echo '<td data-label="Faturado">' . $faturado . '</td>';
            echo '<td data-label="Ações" style="text-align:left">';
