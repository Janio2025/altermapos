...
<style>
  ...
  @media (max-width: 768px) {
    table#tabela-os thead {
      display: none;
    }

    table#tabela-os tbody tr {
      display: block;
      margin-bottom: 15px;
    ...
    table#tabela-os tbody tr td::before {
    ...
    table#tabela-os tbody tr td:last-child {
      display: flex;
      ...
      <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aOs')) { ?>
        <div class="span3">
          <a href="<?php echo base_url(); ?>index.php/os/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px">
            <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2">Ordem de Serviço</span>
          </a>
        </div>
      <?php } ?>
        ...
        <table id="tabela-os" class="table table-bordered">
          ...
          <tbody>
            ...
            foreach ($results as $r) {
              ...
              echo '<tr>';
              echo '<td data-label="N°">' . $r->idOs . '</td>';
              echo '<td data-label="Cliente"><a href="' . base_url() . 'index.php/clientes/visualizar/' . $r->idClientes . '" style="margin-right: 1%">' . $r->nomeCliente . '</a></td>';
              echo '<td data-label="Responsável">' . $r->nome . '</td>';
              echo '<td data-label="Data Inicial">' . $dataInicial . '</td>';
              echo '<td data-label="Data Final">' . $dataFinal . '</td>';
              echo '<td data-label="Venc. Garantia"><span class="badge" style="background-color: ' . $corGarantia . '; border-color: ' . $corGarantia . '">' . $vencGarantia . '</span> </td>';
              echo '<td data-label="Valor Total">R$ ' . number_format($r->totalProdutos + $r->totalServicos, 2, ',', '.') . '</td>';
              echo '<td data-label="Valor com Desconto">R$ ' . number_format(floatval($r->valor_desconto), 2, ',', '.') . '</td>';
              echo '<td data-label="V.T (Faturado)">R$ ' . number_format($r->valor_desconto != 0 ? $r->valor_desconto : $r->valorTotal, 2, ',', '.') . '</td>';
              echo '<td data-label="Status"><span class="badge" style="background-color: ' . $cor . '; border-color: ' . $cor . '">' . $r->status . '</span> </td>';
              ...
              if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
                ...
                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/os/imprimirLaudo/' . $r->idOs . '" target="_blank" class="btn-nwe6" title="Imprimir Laudo"><i class="bx bx-printer bx-xs"></i></a>';
