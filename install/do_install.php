...
if (! empty($_POST)) {
    ...
    $full_rg = $_POST["full_rg"];
    $full_cpf = $_POST["full_cpf"];
    $email = $_POST['email'];
    $login_password = $_POST['password'] ? $_POST['password'] : '';
    $base_url = $_POST['base_url'];

    //check required fields
    if (! ($host && $dbuser && $dbname && $full_name && $full_rg && $full_cpf && $email && $login_password && $base_url)) {
    ...
    $sql = str_replace('admin_rg', $full_rg, $sql);
    $sql = str_replace('admin_cpf', $full_cpf, $sql);
